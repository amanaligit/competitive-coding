...
double clustering(vector<int> x, vector<int> y, int k)
{
  ...
  priority_queue<pair<double, pair<int, int>>> distances;
  for (size_t i = 0; i < x.size(); i++)
  {
    for (size_t j = 0; j < x.size(); j++)
    {
      if (i != j)
      {
        distances.push(make_pair(-sqrt((x[i] - x[j]) * (x[i] - x[j]) + (y[i] - y[j]) * (y[i] - y[j])), make_pair(i, j)));
      }
  ...
  {
    pair<double, pair<int, int>> edge = distances.top();
    distances.pop();
    if (points.getParent(edge.second.first) != points.getParent(edge.second.second))
    {
      points.merge(edge.second.first, edge.second.second);
      // ans = distances[i + 1];
      num_clusters--;
    }
  }
