...
{
    vector<long> sums(queries.size());
    vector<long long> pre(numbers.size() + 1);
    vector<long long> pre0(numbers.size() + 1);
    pre[0] = 0;
    pre0[0] = 0;
    for (size_t i = 0; i < numbers.size(); i++)
    {
        pre[i + 1] = pre[i] + numbers[i];
        pre0[i + 1] = pre0[i] + int(numbers[i] == 0);
    }

    for (size_t i = 0; i < queries.size(); i++)
    {
        long long sum = pre[queries[i][1]] - pre[queries[i][0] - 1];
        long long zero = pre0[queries[i][1]] - pre0[queries[i][0] - 1];
        sums[i] = sum + zero * queries[i][2];
    }
    return sums;
}
